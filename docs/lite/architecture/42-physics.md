...
- `applyPhysicsBodyInstanceImpulse(world, body, index, impulse, location)` and
  `getPhysicsBodyInstanceLinearVelocityToRef(world, body, index, result)` address
  exactly one native thin-instance body. Their explicit index is validated as a
  nonnegative integer within the body count; ordinary bodies accept only index
  zero. Foreign-world and removed bodies are rejected before any native call.
  The instance-zero path uses the lazy context's stable cloned handle, never the
  primary facade handle, so it cannot accidentally broadcast.
