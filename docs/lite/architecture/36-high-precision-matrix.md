...
The public `Mat4` interface is opaque, read-only, and branded so users cannot fabricate or accidentally write to matrices vended by the engine. Internal kernels (`multiplyMat4`, `invertMat4`, `packMat4IntoF32`, allocators) operate on the raw `Mat4Storage = Float32Array | Float64Array` union, which is writable and brand-free. The two types describe the same memory; you cross between them via `as unknown as Mat4Storage` / `as unknown as Mat4` at the trust boundary. See `21-core-math.md` for full type details.
