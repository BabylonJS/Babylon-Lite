...
When the shared walk first reaches a Phase 4 class or Int input, it dynamically imports the Phase 4 enabler, propagates value types, and installs the graph-local evaluator resolver before any type-dependent evaluator selection. Direct parser output and explicitly graph-plumbing-normalized graphs therefore follow the same Phase 4 path through every builder. Candidate-free and detached-candidate graphs do not fetch the enabler or evaluator registry.
...
`NodeParticleSet._graph` remains exactly the graph supplied to the selected builder. Builders do not perform topology normalization or replace it. Phase 4 builder preflight may annotate that same graph with propagated connection value types and an internal evaluator resolver; graph-plumbing normalization remains explicit for direct callers and automatic at the snippet boundary.
...
Phase 4A measured scenes 262, 264, 280, 281, 302, and 305 against base commit `69c8588f`. Final raw/gzip totals are `38.7/24.1 KB`, `38.5/25.9 KB`, `40.4/25.9 KB`, `40.2/26.1 KB`, `53.0/22.1 KB`, and `41.2/26.5 KB`, respectively. Exact raw totals are `39,657`, `39,374`, `41,319`, `41,207`, `54,237`, and `42,144` bytes: changes of `+274`, `+300`, `+334`, `+381`, `+282`, and `+67` bytes from the base manifest. The largest transparent-preflight movement is therefore `381` raw bytes, with no feature evaluator retained by a non-user scene and every measured scene remaining below its ceiling. The accepted topology leaves `npe-registry-extra-values.ts` unchanged, detects reachable Phase 4 use during the shared walk, and installs a graph-local resolver whose enabler and evaluator import table are separate lazy modules. All six control scenes fetch neither Phase 4 module nor any Phase 4A evaluator body.
