...
Opt-in feature adapters call this same walk with block and input evaluator hooks. Each thin texture-update runtime supplies one class, one texture input name, its update evaluator, and `cpuTextureSourceBlock`. Flow maps select `UpdateFlowMapBlock.flowMap`; noise selects `UpdateNoiseBlock.noiseTexture`. Feature block overrides have precedence, followed by scalar Random, local shapes, serialized variants, graph-local optional resolvers, and the standard registry. Every non-feature block therefore follows exactly the same routing in every builder. The ordinary builder and every shared registry remain unchanged for scenes that do not call an opt-in texture-update builder.
