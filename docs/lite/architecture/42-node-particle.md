...
Phase 4A raises the normalized-builder class count from 34 to 37, adds three evaluator files, one shared value-type helper, and one nested Phase 4 value registry. The current descriptive sections are synchronized at 37 normalized-builder classes, 11 particle root files, 30 node infrastructure files, and 51 evaluator/helper files.
...
Phase 4A measured scenes 262, 264, 302, and 305 before and after implementation. Final raw/gzip totals are `39.5/24.5 KB`, `39.2/26.2 KB`, `52.7/21.9 KB`, and `42.1/26.8 KB`, respectively. The three scenes that execute ParticleMath add `1.0 KB` raw and `0.4–0.5 KB` gzip for shared recursive type resolution and Babylon-compatible Int-result coercion; Scene 302, which does not execute ParticleMath, remains byte-identical. Direct Phase 4 cases in `npe-registry-extra-values.ts` caused an unrelated Scene 305 routing cost, so the accepted topology delegates value-registry misses to the unfetched nested `npe-registry-phase4-values.ts`. All four non-user scenes fetch neither that registry nor any Phase 4A evaluator body.
