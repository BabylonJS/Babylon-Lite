...
- Scalar OncePerParticle Random imports its evaluator directly from the builder. Both serialized `number` and normalized `int` metadata select this scalar path in the default, flow-map, and texture-update walks; vector and color tags alone select the typed evaluator.
...
All three evaluators validate required connections during build, reject unknown operations during build, and reject unsupported runtime shapes deterministically. They add no particle storage, steps, callbacks, or public API. The normalizer installs a graph-local optional resolver only after detecting a Phase 4 class or Int source. The resolver and evaluator import table are separate lazy modules, so existing value-block users fetch neither Phase 4 routing metadata nor evaluator bodies. The default, flow-map, and texture-update walks consult this resolver only after their direct scalar, local-shape, serialized-variant, and feature evaluator choices, preserving specialized overrides while composing Phase 4 with every builder family. Unit coverage includes every operation, defaults, numeric edge cases, all supported shapes, scratch identity and intra-evaluation alias safety, diagnostics, parser-derived Int/Float metadata, complete supported Int propagation, mixed Int/Float Math typing and Int-left coercion, Int OncePerParticle routing, specialized-builder composition, and complete parsed-graph execution. Bundle coverage rejects the Phase 4 enabler, registry, type resolver, Int Math wrapper, and all three evaluator modules in representative non-user scenes, including scene305, and rejects folding evaluator bodies into the parser, builder, or shared registry.
...
Phase 4A measured scenes 262, 264, 280, 281, 302, and 305 against base commit `69c8588f`. Final raw/gzip totals are `38.4/24.0 KB`, `38.1/25.7 KB`, `40.0/25.8 KB`, `39.9/25.9 KB`, `52.7/21.9 KB`, and `41.1/26.4 KB`, respectively. Exact raw totals are `39,368`, `39,059`, `40,972`, `40,811`, `53,938`, and `42,064` bytes: changes of `-15`, `-15`, `-13`, `-15`, `-17`, and `-13` bytes from the base manifest. Recursive type resolution, Int-result coercion, and specialized-builder composition therefore add no runtime-byte regression to representative non-user scenes. The accepted topology leaves `npe-registry-extra-values.ts` unchanged, detects Phase 4 use during graph normalization, and installs a graph-local resolver whose enabler and evaluator import table are separate lazy modules. All six control scenes fetch neither Phase 4 module nor any Phase 4A evaluator body.
