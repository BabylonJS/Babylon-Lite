...
plain state to standalone subsystem functions. Indexed impulse, velocity, and
reset helpers live in `playroom/physics-instances.ts`; they use the internal
thin-instance context and do not increase ordinary physics scene bundles.

The only additional root-exported opt-in is:

```ts
function enableHavokThinInstanceAdvancedPhysics(world: PhysicsWorld): void;
```

It installs per-instance scaled-shape and mass handling before Playroom bodies
are created. Consumers that do not opt in retain the smaller shared path.
