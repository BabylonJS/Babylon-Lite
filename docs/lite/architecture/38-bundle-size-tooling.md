...
file per scene under `lab/public/bundle/manifest/`. The single aggregate
`manifest.json` is a generated (gitignored) build artifact that runtime consumers
(lab UI, bundle-size test, report script, static lab site) read; on a fresh
checkout the lab dev server synthesizes it on the fly from the per-scene files.

Splitting the baseline per scene was meant to stop PRs colliding on one shared
file, and it does help for scene-local changes. It does not help for the common
case: almost any change to a shared module moves bytes in most of the ~230
scenes, so two such branches still rewrite the same ~200 files. That is why the
files are now written only by master (see below) rather than by PR authors.
