...
| ----------------------------------------- | ---------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lab/public/bundle/manifest/<scene>.json` | `pnpm build:bundle-scenes`                                       | Yes      | Current/local per-scene runtime measurements — the tracked repository bundle baseline. **Refreshed automatically by CI** (see below); commit regenerated files yourself only if you want to. |
...
CI does. Nearly every change to a shared module moves bytes in most of the ~230
scenes, so requiring the author to commit the regenerated files made the Bundle
Size job structurally unstable: the moment any shared-code PR merged, every other
open PR's committed manifest was stale and its job went red for reasons unrelated
to that PR — curable only by a rebase plus a very slow full local rebuild, then
again on the next merge to master.
...
3. pushes the refreshed per-scene files to the PR branch as a bot commit
   (`pnpm commit:bundle-manifest`), so the size deltas still appear in the PR diff.

`scripts/commit-bundle-manifest.ts` adds a detached worktree at the PR **head** and
commits there — an Azure PR build checks out the PR _merge_ commit, and pushing
that onto the source branch would silently merge master into the contributor's
branch. It refuses to run for fork PRs (no push access), refuses to mirror a
partial build, and stops after two consecutive bot commits rather than
ping-ponging on a non-deterministic measurement. Every failure is a warning; the
step can never be the reason a PR is red.
...
| `scripts/commit-bundle-manifest.ts`    | Pushes the refreshed per-scene manifest to the PR branch so the baseline never goes stale.                    |
