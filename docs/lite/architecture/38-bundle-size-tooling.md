...
### Three size metrics per scene

Each per-scene manifest records three views of the runtime bundle:

| Field(s)                                                                   | Metric            | Meaning                                                                                                                                                              |
| -------------------------------------------------------------------------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rawKB` / `gzipKB` / `runtimeChunks`                                        | **First frame**   | JS actually fetched to render the first frame. This is the **gated** ceiling metric (`bundle-size.spec.ts` vs `maxRawKB`). `startEngine` resolves after the first RAF frame, and every scene sets `canvas.dataset.ready = "true"` immediately after, so "fetched at `ready`" == first-frame bytes. |
| `firstFrameRawKB` / `firstFrameGzipKB`                                      | **First frame**   | Explicit aliases of `rawKB` / `gzipKB` for clarity in tooling.                                                                                                        |
| `fullExperienceRawKB` / `fullExperienceGzipKB` / `fullExperienceRuntimeChunks` | **Full experience** | First-frame bytes **plus** interaction-only lazy chunks that a scene fetches only when the user exercises a feature (pick, gizmo, dispose, animation, …). Equals the first-frame numbers when a scene exposes no exercise hook. Informational only — **not** gated. |

The **full-experience** number is a *true runtime* measurement, not a static
sum of every emitted chunk: the harness keeps the page alive after the first
frame, triggers the scene's own interaction paths, and measures the extra lazy
JS that is actually fetched.

### Scene-side `__bjsExerciseFull` convention

A scene that has interaction-only features (chunks not fetched at first frame)
opts into a distinct full-experience number by exposing an async hook:

```ts
(window as unknown as { __bjsExerciseFull?: () => Promise<void> }).__bjsExerciseFull = async () => {
    // Trigger the scene's interaction-only lazy paths (e.g. a GPU pick that
    // dynamic-imports the picker chunk, attaching a gizmo, disposing a mesh).
    await doTheInteraction();
    canvas.dataset.fullReady = "true"; // signal completion to the harness
};
```

The harness (`measurePage`) snapshots the first-frame payload set at
`dataset.ready`, then invokes `window.__bjsExerciseFull()` (if present), waits
for `dataset.fullReady === "true"` (bounded timeout) plus a brief network
settle, and snapshots the full payload set. Scenes **without** the hook produce
no extra fetches, so their full-experience totals equal their first-frame
totals. Adoption is incremental — no need to script all scenes at once.
See `lab/lite/src/lite/scene222.ts` for a reference implementation (its GPU
picker is `import("babylon-lite")`-ed only inside the pick handler, so the
picker chunk is a genuine post-first-frame lazy fetch).

#### Limitation: first frame measures *fetched* bytes, not *needed* bytes

The first-frame number is "JS bytes fetched before `dataset.ready`", not "the
minimal JS the first frame strictly needs". These differ for any code that is
**statically imported** into an eagerly-fetched chunk but not exercised until
later. Example: a scene that `import`s `removeFromScene` at module top puts that
function in the entry chunk, which is fetched to reach the first frame — so its
bytes count toward first-frame even though nothing calls it before `ready`.
Conversely, code reached only through `import(...)` (dynamic import) is not
fetched until its trigger runs, so it lands in the full-experience delta (or
nowhere, if never triggered).

The runtime-fetch metric therefore **cannot** isolate "statically bundled but
not needed for the first frame" from "genuinely needed for the first frame" —
that is a static-vs-dynamic-import *architectural* property of the code, not
something the network measurement can see. To move such code out of the
first-frame number, convert its import from static to dynamic (`import(...)`)
behind the interaction that needs it; it then shows up only in full-experience.
Read the two numbers accordingly: first-frame = "what a cold load pays to render
frame 1", full-experience = "what the session pays once the measured
interactions have run" — neither claims to be the theoretical minimum.
