...
- transparent meshes follow the geometry task's normal mesh list; callers that want opaque-only tags pass a filtered `meshes` array;
...
- source and target must not alias;
...
- Creating a mesh-blending task explicitly installs a PBR scene hook and shader fragment for Babylon.js-compatible non-uniform normal handling. The hook, feature detection, inverse-matrix WGSL, and extra tangent-frame varying live in `mesh-blending-pbr-support.ts`; ordinary PBR scenes do not import or retain them.
- Creating the task also installs the geometry mesh-tag resolver. Standard, PBR, and Node geometry renderables resolve tags through the tiny geometry-types hook and do not statically import `mesh-blending-tag.ts`.
...
- A representative ordinary PBR/glTF scene remains byte-identical to the public-master bundle baseline. Geometry-renderer bundles retain only the reusable typed-`r8uint` attachment/output capability; the post-process, tag helpers, blue noise, search shader, and non-uniform PBR correction are absent until mesh blending is imported and constructed.
...
No golden or thumbnail is generated by this task because parity execution is explicitly deferred to the parent.
