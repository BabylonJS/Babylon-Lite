...
- **Bundle size**: pure-2D scenes import `SpriteRenderer` only; depth-hosted support stays behind the `addToScene` sprite admission branch and lazy `sprite-renderable` import.
