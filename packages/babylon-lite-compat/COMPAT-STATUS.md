...
| BJS API                                                                                                         | Status             | Module                                                                                                                                                                                                                                     |
| --------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Node` (base) + `getScene`/`getClassName`/`parent`/`metadata` + `getDescendants`/`getChildren`/`getChildMeshes` + `isEnabled(checkAncestors)`/`setEnabled` + `onEnabledStateChangedObservable`/`onEffectiveEnabledStateChangedObservable` | ⚡ Partial         | [node/node.ts](src/node/node.ts) (scene-graph traversal over a child registry maintained by the `parent` setter / `setParent`; ancestor-aware `isEnabled`, and the own + effective enabled-state observables — the latter new in BJS `9.17` — fire on `setEnabled` for the node and every descendant whose effective state flips) |
