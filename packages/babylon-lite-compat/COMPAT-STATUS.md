...
| Loaded-camera surfacing / KTX2 textures                     | 112                                                                                         | **24** now works (MAD ≈ 0): a camera parsed from a `.babylon` file is surfaced as the compat `scene.activeCamera` via `FreeCamera._adopt` (the loader calls `scene._surfaceLoadedCamera()` after `addToScene`), and `camera.attachControl` wires Lite's `attachFreeControl` with the scene. **112** still throws `scene.materials is not iterable` (needs a `scene.materials` accessor) and underneath requires KTX2 / `KHR_texture_basisu` transcode — the same basis blocker as scene 36.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
