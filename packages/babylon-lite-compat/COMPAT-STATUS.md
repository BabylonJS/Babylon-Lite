...
| BJS API                                                                    | Status           | Module                                                                                                                                               |
| -------------------------------------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Texture` (2D, URL)                                                        | ✅ Full          | [textures/textures.ts](src/textures/textures.ts) (async load awaited at build; honours `invertY` / `noMipmap` / `samplingMode` on the material path; `.basis` URLs route through Lite `loadBasisTexture2D`) |
...
### ✅ Working (103 scenes, MAD ≈ 0)

`1`, `2`, `4`, `5`, `6`, `9`, `10`, `11`, `13`, `14`, `15`, `16`, `18`, `19`, `22`, `24`, `28`,
...
`158`, `170`, `171`, `172`, `174`, `175`, `200`, `201`, `202`, `203`, `204`, `205`, `207`, `210`, `213`, `216`, `218`,
`219`, `221`, `222`, `223`, `224`
...
| Navigation tile-cache obstacles (Recast)                    | 173                                                                                    | **172** now works (MAD ≈ 0): the compat navigation wrapper gained the tile-cache obstacle surface — `plugin.addCylinderObstacle` / `addBoxObstacle` / `removeObstacle` over Lite's `addCylinderObstacle`/`addBoxObstacle`/`removeObstacle`, `createNavMesh` now returns `{ navMesh, tileCache }`, and a standalone `WaitForFullTileCacheUpdate` delegates to Lite's `updateNavMeshObstacles`. **173** also exercises this surface but additionally removes an obstacle on a `setTimeout` and rebuilds the nav debug mesh + path mid-run, so it needs the dynamic-removal/rebuild sequence verified for parity.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Navigation meshes (Recast)                                  | 173                                                                                    | Recast nav is wrapped: scenes **170, 171, 172, 174, 175** work (MAD ≈ 0) via a compat `@babylonjs/addons/navigation` wrapper (`navigation/navigation.ts`) over Babylon Lite's native Recast API — navmesh build, debug-mesh visualization, crowd + `addAgent`/`agentGoto`/`timeFactor`, `computePath`, `raycast`, off-mesh connections (174), and tile-cache obstacles (172). The `@recast-navigation/*` packages the oracle imports are shimmed to no-ops since Lite loads its own Recast wasm. Also: glTF `result.meshes` now includes the BJS `__root__` at index 0 (so `result.meshes[1]` resolves), and `mesh.bakeCurrentTransformIntoVertices()` folds a node's transform into its CPU geometry. **173** remains — it needs the dynamic obstacle-removal + nav-debug/path rebuild sequence verified for parity.                                                                                                                                                                                                                                                                                                                                                                     |                                                                                                                                                                                                                                                                                                                                                                     |
...
| Loaded-camera surfacing / KTX2 textures                     | 112                                                                                         | **24** now works (MAD ≈ 0): a camera parsed from a `.babylon` file is surfaced as the compat `scene.activeCamera` via `FreeCamera._adopt` (the loader calls `scene._surfaceLoadedCamera()` after `addToScene`), and `camera.attachControl` wires Lite's `attachFreeControl` with the scene. **112** still throws `scene.materials is not iterable` (needs a `scene.materials` accessor) and underneath requires KTX2 / `KHR_texture_basisu` transcode (the `.ktx2` container path; the `.basis` path itself is wrapped — see scene 36).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
