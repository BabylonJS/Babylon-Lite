...
| Materials (`StandardMaterial`, `PBRMaterial`, metallic-rough / spec-gloss, `NodeMaterial`)                                                |   ⚡   | `clone(name)` shares textures with its own renderable; custom WGSL `MaterialPluginBase` injection is supported, while `DitheredTileFadeMaterialPlugin`, GLSL `ShaderMaterial`, `MultiMaterial`, `BackgroundMaterial`, and `OpenPBRMaterial` remain unsupported                                                                                                                            |
