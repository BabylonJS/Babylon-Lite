...
Compat exports the Babylon.js `IEasingFunction` / `EasingFunction` API and the
standard `CircleEase`, `QuadraticEase`, `CubicEase`, `QuarticEase`,
`QuinticEase`, `SineEase`, `ExponentialEase`, `BackEase`, `ElasticEase`,
`BounceEase`, `PowerEase`, and `BezierCurveEase` classes. Their class and easing
mode behavior remains Babylon.js-shaped, while curve math delegates to Lite's
tree-shakable functional easing exports.

For supported float, vector, and quaternion tracks, `beginAnimation` and
`beginDirectAnimation` translate the compat animation into native Lite property
animation. Lite then owns timing, sampling, interpolation, writes, and mixing.
Unsupported track semantics use the explicit compat fallback; structural compat
`AnimationGroup` playback remains compat-owned. Delegated groups participate in
the native manager only while playing, and mixed native/fallback facades preserve
per-track completion and aliased-property write order.
...
| Engine (`WebGPUEngine`, `Engine`, `ThinEngine`, `NullEngine`)                                                                             |   ⚡   | async startup + render loop; `currentSampleCount` reports the active surface's MSAA count; `beginFrame`/`endFrame`, engine-wide alpha-to-coverage, and manual `scene.render()` unsupported                                                                                                                                                                                                                                                                                                                                           |
...
| Meshes & geometry (class chain, `MeshBuilder` primitives, transforms, thin instances, `clone`, `VertexData`, `Mesh.MergeMeshes`)          |   ⚡   | `CreateLines`/`CreateLineSystem` support fixed-topology line lists and RGBA point colors; `CreateDashedLines` supports uniform-color dashed line lists; mesh-blending tag helpers and validated mesh metadata work, while the visual blending pipeline remains unsupported; `MergeMeshes` bakes positions/normals/UVs and rejects unsupported attributes/animation; `CreateDecal`/`CreateText`/`CreateTiledBox`/`CreateTiledPlane`, `GreasedLine*` (thick lines), `InstancedMesh`, LOD/edges/outline, hardware instances unsupported |
...
| Post-processes, layers (glow/highlight), probes, WebXR                                                                                    |   ❌   | not in Babylon Lite — use native Lite `create*Task` functions; Babylon.js mesh blending additionally needs geometry tag/depth/albedo outputs that Lite does not expose                                                                                                                                                                                                                                                                                                                                                               |
...
| OpenUSD (`USDFileLoader`)                                                                      |   ❌   | named stub; requires a WebAssembly worker and a USD-to-Lite scene/material materializer                    |
