...
| Physics (`HavokPlugin`, `scene.enablePhysics`/`getPhysicsEngine`, `PhysicsAggregate`/`PhysicsBody`/`PhysicsShape`) |   ⚡   | Havok V2 over Lite `createHavokWorld`; refresh-rate-independent `useDeltaForWorldStep`; common body/shape construction and mutation forward to Lite's native physics API |
