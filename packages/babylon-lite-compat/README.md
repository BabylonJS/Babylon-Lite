...
| Cameras (`ArcRotateCamera`, `FreeCamera`/`Universal`/`Target`, `FollowCamera`, `GeospatialCamera`)      |   ✅   | Control via `attachControl`; XR / device-orientation / stereoscopic rigs and the per-input `camera.inputs` handler classes unsupported |
