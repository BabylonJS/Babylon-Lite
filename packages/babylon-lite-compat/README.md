...
| Meshes & geometry (class chain, `MeshBuilder` primitives, transforms, thin instances, `VertexData`, `Mesh.MergeMeshes`) |   ⚡   | `MergeMeshes` bakes positions/normals/UVs (rejects multi-material/submesh merges, colours/tangents/UV2, skinning); `CreateLines`/`CreateDecal`/`CreateText`, `InstancedMesh`, LOD/edges/outline, clone/instance unsupported |
