...
| Meshes & geometry (class chain, `MeshBuilder` primitives, transforms, thin instances, `clone`, `VertexData`) |   ⚡   | `CreateLines`/`CreateDecal`/`CreateText`/`CreateTiledBox`/`CreateTiledPlane`, `InstancedMesh`, LOD/edges/outline, hardware instances unsupported |
...
| Textures (`Texture`, `RawTexture`, `RawTexture2DArray`, `RawTexture3D`, `DynamicTexture`, `CubeTexture`, `HDRCubeTexture`) |   ⚡   | `HDRCubeTexture` routes to native `loadHdrEnvironment`; `RawTexture2DArray` supports the raw-bytes, image-source/URL, and single-file KTX2 paths; `RenderTargetTexture` / `MirrorTexture` / `HtmlTexture` unsupported |
