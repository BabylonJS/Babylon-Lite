...
| Textures (`Texture`, `RawTexture`, `RawTexture2DArray`, `RawTexture3D`, `DynamicTexture`, `CubeTexture`, `HDRCubeTexture`) |   ⚡   | `HDRCubeTexture` routes to native `loadHdrEnvironment`; `RawTexture2DArray` supports the image-source/URL path (raw-bytes upload throws); `RenderTargetTexture` / `MirrorTexture` / `HtmlTexture` unsupported |
