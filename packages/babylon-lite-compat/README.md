...
| Textures (`Texture`, `RawTexture`, `RawTexture2DArray`, `RawTexture3D`, `DynamicTexture`, `CubeTexture`, `HDRCubeTexture`) |   ⚡   | `HDRCubeTexture` routes to native `loadHdrEnvironment`; `RawTexture2DArray` supports raw bytes, image-source/URL paths, and KTX2 arrays; `RenderTargetTexture` / `MirrorTexture` / `HtmlTexture` unsupported |
