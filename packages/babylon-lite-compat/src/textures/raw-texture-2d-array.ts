...
/**
 * Babylon.js `CreateTexture2DArrayFromKTX2Async` — decode a single KTX2 container
 * holding several array layers into a {@link RawTexture2DArray}.
 *
 * 🔧 Needs Lite core. Structural blocker: unlike the image-URL sibling
 * ({@link CreateTexture2DArrayFromImageUrlsAsync}, backed by Lite's
 * `createTexture2DArrayFromUrls`), there is no Lite primitive that decodes a KTX2
 * container into per-array-layer RGBA bytes. Lite's KTX2 decoder
 * (`texture/ktx2-loader.ts`) models **single-image 2D textures only**: its
 * `Ktx2DecodedData`/`Ktx2DecodedMip` types expose neither `layerCount` nor a
 * per-mip `layerIndex`, and its only RGBA output helper
 * (`decodeKtx2ImageBitmapFromBuffer`) returns the base mip of layer 0 as one
 * ImageBitmap. Surfacing the array layers would require extending that decoder's
 * typed output and adding a decode-to-layers path **inside `ktx2-loader.ts`, which
 * is already pulled into scene bundles** (via `loader-gltf/gltf-ext-basisu.ts`), so
 * it cannot be added as a purely-additive tree-shakeable export — and how Lite
 * should model multi-layer KTX2 decode output is a design decision for the Lite
