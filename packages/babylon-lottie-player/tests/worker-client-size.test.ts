...
it.each([
    ...
    ["fixed shapes splash client", "fixed-shapes-splash/client.min.js", 521, 374],
...
it("sets the JavaScript MIME type on the emitted fixed shapes splash worker bootstrap", () => {
    const output = readFileSync(resolve(distDir, "fixed-shapes-splash", "client.min.js"), "utf8");
    expect(output).toMatch(/new Blob\(\[[^\]]*\],\s*\{\s*type:\s*"text\/javascript"\s*\}\)/);
});
