/** Scene-specific WebGPU device-loss recovery. */
...
export interface DeviceLostSceneRecoveryHandle {
    disable(): void;
}
...
/** Enable device-loss recovery for every SceneContext registered on the engine's surfaces. */
export function enableDeviceLostSceneRecovery(engine: EngineContext, options: DeviceLostSceneRecoveryOptions = {}): DeviceLostSceneRecoveryHandle {
    return _enableDeviceLostRecovery(engine, {
        _kind: "scene",
        _recover: recoverSceneContexts,
        _enable: attachRecoveryCapture,
        _disable: detachRecoveryCapture,
        _callbacks: options,
    });
}

async function recoverSceneContexts(engine: EngineContext): Promise<void> {
    clearSceneBGLCache();
    for (const surface of engine.surfaces) {
        for (const context of surface._renderingContexts) {
            if (context._kind === "scene" && isRenderingContextRegistered(surface, context)) {
                await rebuildSceneGpu(engine, context as SceneContext);
            }
        }
    }
}
...
async function rebuildTexture2D(engine: EngineContext, tex: Texture2D): Promise<void> {
    ...
    if (source.kind === "url") {
        Object.assign(tex, await rebuildUrlTexture2D(engine, source.url, source.opts));
        return;
    }
