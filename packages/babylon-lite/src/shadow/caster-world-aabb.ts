...
let _morphCasterAabb: ((mesh: Mesh) => Aabb | null) | null = null;
...
/** @internal Fold local bounds through a mesh world matrix using the shared scratch storage. */
export const _worldAabbFromLocalBounds = worldAabbFromLocalBounds;

/** @internal Install the lazily loaded skeletal bounds implementation. */
export function _installSkinnedCasterAabb(skinned: (mesh: Mesh) => Aabb | null): void {
    _skinnedCasterAabb = skinned;
    _casterBoundsVersion++;
}

/** @internal Install the lazily loaded morph bounds implementation. */
export function _installMorphCasterAabb(morph: (mesh: Mesh) => Aabb | null, morphLocal: (mesh: Mesh) => Aabb | null): void {
    _morphCasterAabb = morph;
...
export function casterWorldAabb(mesh: Mesh): Aabb | null {
    ...
    if (mesh.morphTargets) {
        ...
        const morphed = _morphCasterAabb(mesh);
        if (morphed) {
            return morphed;
        }
    }
