...
export function createTaaPostProcessTask(config: TaaPostProcessTaskConfig, engine: EngineContext, scene?: SceneContext): TaaPostProcessTask {
    ...
    const samples = normalizeSampleCount(config.samples);
    ...
    const task: TaaTaskInternal = {
        ...
        record(): void {
            ...
            const sourceSize = resolveRenderTargetSize(task.sourceTexture);
            ensurePersistentTarget(history, engine, sourceSize.width, sourceSize.height);
            ensurePersistentTarget(temp, engine, sourceSize.width, sourceSize.height);
            blend.record();
            present.record();
        ...
        execute(): number {
            ...
            // jittered viewProjection into the source task's UBO. Reset only affects the
            // blend factor for the frame that just rendered; the next frame can jitter.
            advanceJitter(task);
            return draws;
        },
        updateUniforms(): void {
...
function normalizeSampleCount(samples: number | undefined): number {
    if (samples === undefined || !Number.isFinite(samples)) {
        return 8;
    }
    return Math.max(1, Math.floor(samples));
}

function resolveRenderTargetSize(rt: RenderTarget): { width: number; height: number } {
    if (rt._width > 0 && rt._height > 0) {
        return { width: rt._width, height: rt._height };
    }
...
function advanceJitter(task: TaaTaskInternal): void {
    ...
    const { width: w, height: h } = resolveRenderTargetSize(task.sourceTexture);
    if (w <= 0 || h <= 0) {
        return;
    }
