...
/**
 * `ParticleTextureSourceBlock` — loads the particle texture from its `url` (or its embedded
 * `textureDataUrl`) and binds it onto the system for the billboard renderer. The load is registered as a
 * build promise so the set is only ready once it settles. Relative URLs use the build's texture base URL,
 * and the serialized invert-Y flag is converted to the texture loader's upload convention.
 */
