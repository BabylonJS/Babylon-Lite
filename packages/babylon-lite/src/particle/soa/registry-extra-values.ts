...
export async function loadValueBlockEvaluator(className: string): Promise<SoaBlockEvaluator> {
    switch (className) {
        ...
        default:
            throw new Error(`SoA NodeParticle: unsupported value block "${className}"`);
    }
}
