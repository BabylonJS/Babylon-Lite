...
export function lerpGetters(left: SoaGetter, right: SoaGetter, i: number, amount: number, vector2: Vec2, vector3: Vec3, color4: Color4): SoaValue {
    const start = left(i);
...
/** `ParticleLerpBlock` (SoA) — component-wise linear interpolation `left + (right - left) * gradient`, into scratch. */
export const particleLerpBlock: SoaBlockEvaluator = {
    build(block, ctx) {
