...
import { makeContextualGetter } from "../contextual.js";
...
function parseConstant(type: number, value: unknown): SoaValue {
...
/** `ParticleInputBlock` (SoA) — a constant, a contextual source, or a system source. Mirrors the object version. */
export const particleInputBlock: SoaBlockEvaluator = {
    build(block, ctx) {
        ...
        if (contextual !== 0) {
            ...
            switch (systemSource) {
                ...
                case SYS_EMITTER:
                    ...
                    break;
                default:
                    throw new Error(`SoA NodeParticle: unsupported system source ${systemSource}`);
            }
        } else {
