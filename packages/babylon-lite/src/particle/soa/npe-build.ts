...
import type { ParticleGraph, ParsedParticleBlock, ParsedParticleInput } from "../node/npe-types.js";
import type { ParticleBuffer } from "./particle-buffer.js";
import { createSoaSystem, type SoaSystem } from "./animate.js";
...
export interface SoaBuildState {
    system: SoaSystem | null;
...
/** Build context handed to each SoA block evaluator. */
export interface SoaBuildContext {
    state: SoaBuildState;
    engine: EngineContext;
    input(block: ParsedParticleBlock, name: string, fallback?: SoaGetter): SoaGetter;
    isConnected(block: ParsedParticleBlock, name: string): boolean;
    setOutput(blockId: number, name: string, getter: SoaGetter): void;
    addBuildPromise(promise: Promise<void>): void;
}

/** A SoA block evaluator: wires a parsed block into the runtime during the build walk. */
export interface SoaBlockEvaluator {
    build(block: ParsedParticleBlock, ctx: SoaBuildContext): void;
}

/** A built data-oriented particle set. */
export interface SoaParticleSet {
    readonly systems: SoaSystem[];
    /** @internal */
...
export async function buildSoaParticleSet(engine: EngineContext, scene: SceneContext, graph: ParticleGraph, options: BuildSoaOptions = {}): Promise<SoaParticleSet> {
    ...
    for (const systemId of graph.systemBlockIds) {
        ...
        const system = createSoaSystem(capacity);
        ...
        const outputs = new Map<string, SoaGetter>();
        const built = new Set<number>();

        const ctx: SoaBuildContext = {
            state,
            engine,
            input(block, name, fallback) {
                ...
                if (isInputConnected(input)) {
                    ...
                    throw new Error(`SoA NodeParticle: unresolved connection ${block.className}.${name}`);
                }
                if (input) {
                ...
                return fallback ?? (() => null as unknown as SoaValue);
            },
        ...
        const buildBlock = async (blockId: number): Promise<void> => {
            ...
            const evaluator = scalarOnce
                ...
                : localShape
                  ? await (await import("./registry-local-shapes.js")).loadLocalShapeEvaluator(block.className)
                  : variant
                    ? await (await import("./registry-variants.js")).loadVariantBlockEvaluator(block)
                    : await loadSoaBlockEvaluator(block.className);
