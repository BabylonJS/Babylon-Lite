...
import type { ParsedParticleBlock } from "../node/npe-types.js";

/** Resolve serialized variants that should add zero loader bytes to ordinary block instances. */
export async function loadVariantBlockEvaluator(block: ParsedParticleBlock): Promise<SoaBlockEvaluator> {
    switch (block.className) {
        ...
        default:
            throw new Error(`SoA NodeParticle: unsupported block variant "${block.className}"`);
    }
}
