...
export async function loadBasicBlockEvaluator(className: string): Promise<SoaBlockEvaluator> {
    switch (className) {
        ...
        default:
            throw new Error(`SoA NodeParticle: unsupported Basic Properties block "${className}"`);
    }
}
