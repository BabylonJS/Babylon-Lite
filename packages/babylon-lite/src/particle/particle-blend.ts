...
function createBlend(key: string, colorSrc: GPUBlendFactor, colorDst: GPUBlendFactor, alphaSrc: GPUBlendFactor, alphaDst: GPUBlendFactor): BillboardBlendDescriptor {
    return {
        _key: key,
        _descriptor: {
...
export function createParticleBlend(mode: number): BillboardBlendDescriptor {
    switch (mode) {
        ...
        case 3:
            return createBlend("p3", "dst", "zero", "one", "one");
        case 4:
            return createBlend("p4", "dst", "zero", "one", "one");
        default:
