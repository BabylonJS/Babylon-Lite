...
function getPipeline(device: GPUDevice, flipY: boolean): GPUComputePipeline {
    ...
    const p = device.createComputePipeline({
        layout: "auto",
        compute: { module: _module!, entryPoint: "main", constants: { [FLIP_Y_CONSTANT_ID]: flipY ? 1 : 0 } },
    });
    if (flipY) {
        _flip = p;
    } else {
