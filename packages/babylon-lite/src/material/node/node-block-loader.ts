 ...
 * Use exported node-block descriptors rather than constructing one from its name. */
...
export function createNodeMaterialBlockLoader(blocks: readonly NodeMaterialBlock[]): NonNullable<ParseNodeMaterialOptions["blockLoader"]> {
    ...
    for (const block of blocks) {
        if (typeof block._load !== "function") {
            throw new Error(`NodeMaterial: invalid block selection "${block.className}": use an exported block descriptor`);
        }
