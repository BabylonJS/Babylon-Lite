...
const vertexSource = wgsl`struct VertexOutput{@builtin(position) position:vec4f,@location(0) shade:f32,};
@vertex fn mainVertex(input:VertexInput)->VertexOutput{
    var out:VertexOutput;
    out.position=shaderSystem.worldViewProjection*vec4f(input.position.xyz,1);
    out.shade=input.position.w;
    return out;
}`;

const fragmentSource = wgsl`struct VertexOutput{@builtin(position) position:vec4f,@location(0) shade:f32,};
@fragment fn mainFragment(input:VertexOutput)->@location(0) vec4f{
    let t=clamp(input.shade*0.9+0.5,0.0,1.0);
    return vec4f(0.1+t*0.35,0.35+t*0.5,0.55+t*0.4,1);
}`;
...
function buildSlab(): Float32Array {
    ...
    for (let c = 0; c < CHUNKS; c++) {
        const ox = (c & 1) * CHUNK_SIZE - CHUNK_SIZE;
        const oz = (c >> 1) * CHUNK_SIZE - CHUNK_SIZE;
...
async function main(): Promise<void> {
    ...
    for (let i = 0; i < CHUNKS; i++) {
        const ox = (i & 1) * CHUNK_SIZE - CHUNK_SIZE;
        const oz = (i >> 1) * CHUNK_SIZE - CHUNK_SIZE;
