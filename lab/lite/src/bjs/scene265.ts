// BJS reference for scene 265 — builds the SAME NPE graph as Lite (the sprite-sheet "Animations 2"
// system), seeds the RNG identically, steps the simulation a fixed number of times, and renders the
// frozen frame with each particle drawn on its animation-sheet cell.
...
(async function () {
    ...
    const set = NodeParticleSystemSet.Parse(SCENE265_NPE_JSON);
    const built = await set.buildAsync(scene);
    const system = built.systems[0] as ParticleSystem;

    // The sprite-sheet cell config comes from the graph (SetupSpriteSheetBlock); supply the sheet image
    // (mipmaps on, no Y-flip — matching the "Animations 2" playground).
