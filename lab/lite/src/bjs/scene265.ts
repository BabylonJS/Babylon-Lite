// BJS reference for scene 265 — builds the SAME NPE graph as Lite (the "Change - Color" system with two
// color gradients), seeds the RNG identically, steps the simulation a fixed number of times, and renders
// the frozen frame.
...
(async function () {
    ...
    const camera = new ArcRotateCamera("cam", -Math.PI / 2, 1.2, 12, new Vector3(0, 0.6, 0), scene);
    camera.minZ = 0.1;
    camera.maxZ = 100;
