        ...
        configureServer(server) {
            server.middlewares.use((req, res, next) => {
                ...
                const liteHtmlCompat = url.match(
                    /^\/((?:scene|bundle-scene|bundle-bjs-scene|babylon-ref-scene|bundle-baseline-scene)\d+|demo-[^/]+|dispose-test|leak-test|material-swap-test|html-texture-test|picking-test|vat-picking-test|billboard-pick-test)\.html$/
                );
                if (liteHtmlCompat) {
