...
Effect.ShadersStore["scene160VertexShader"] = `
...
Effect.ShadersStore["scene160FragmentShader"] = `
precision highp float;
varying vec2 vUV;
...
(async function () {
    ...
    const material = new ShaderMaterial(
        "scene160Shader",
        scene,
        { vertex: "scene160", fragment: "scene160" },
        { attributes: ["position", "uv"], uniforms: ["worldViewProjection"], samplers: ["textureSampler"] }
    );
    material.backFaceCulling = false;
    const texture = new Texture("/textures/nme/ebf71b300f43563f.png", scene, true, true, Texture.NEAREST_SAMPLINGMODE);
