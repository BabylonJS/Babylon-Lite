...
Effect.ShadersStore["scene161VertexShader"] = `
...
Effect.ShadersStore["scene161FragmentShader"] = `
precision highp float;
uniform vec3 tint;
...
(async function () {
    ...
    const material = new ShaderMaterial(
        "scene161Shader",
        scene,
        { vertex: "scene161", fragment: "scene161" },
        { attributes: ["position"], uniforms: ["worldViewProjection", "tint", "intensity"] }
    );
    material.setVector3("tint", new Vector3(1.0, 0.35, 0.1));
    material.setFloat("intensity", 0.85);
