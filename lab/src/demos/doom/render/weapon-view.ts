...
// and showing a muzzle-flash overlay when fired. Here we reproduce it with a
// lite `Sprite2DLayer` drawn by a `SpriteRenderer` overlay (a 2D pass layered on
// top of the scene's swapchain), exercising the engine's Sprite2D custom-shader
// hook: the same palette + COLORMAP lookup the world uses, at the full-bright row.
//
// Weapon sprite lumps are decoded from the WAD's S_START..S_END namespace into a
// single palette-indexed atlas (R = palette index, A = coverage). Placement uses
// the documented psprite formula: a virtual 320x200 frame with the sprite's pivot
// offsets, the ready weapon resting at WEAPONTOP. No GPL Doom source is used.
...
export class WeaponView {
    ...
    constructor(
        ...
        // Layer pivot [0,0] so each sprite's `positionPx` is its top-left corner,
        // matching the virtual-frame placement we compute below.
        this.layer = createSprite2DLayer(this.atlas, { depth: "none", blendMode: "alpha", pivot: [0, 0], capacity: 2, customShader });
        // Gun + flash slots, both hidden until the first `update`.
        addSprite2DIndex(this.layer, { positionPx: [0, 0], sizePx: [0, 0], visible: false });
        addSprite2DIndex(this.layer, { positionPx: [0, 0], sizePx: [0, 0], visible: false });
