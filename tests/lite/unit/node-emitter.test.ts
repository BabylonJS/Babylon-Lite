...
describe("NodeMaterial emitter core", () => {
    ...
    it("keeps same-named serialized inputs as distinct uniforms", async () => {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    inputs: [
                        ...
                        { name: "z", targetBlockId: 5, targetConnectionName: "output" },
                    ],
                    outputs: [{ name: "xyzw" }],
                },
                {
        ...
        expect(result.state.nodeUboFields.map((field) => field._name)).toEqual(["Float", "Float1", "Float2"]);
        expect(result.fragmentWgsl).toContain("nodeU.Float");
        expect(result.fragmentWgsl).toContain("nodeU.Float1");
        expect(result.fragmentWgsl).toContain("nodeU.Float2");
    });
