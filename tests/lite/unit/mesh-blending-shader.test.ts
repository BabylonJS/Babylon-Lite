...
describe("mesh-blending WGSL bindings and coordinates", () => {
    it("uses exact texel loads, a typed integer tag texture, and no samplers", () => {
        ...
        expect(code).toContain("let packed=textureLoad(tagTexture,clampPixel(pixel,size),0).r");
    ...
    it("maps Lite storage coordinates to Babylon.js algorithm coordinates", () => {
        const code = shader();
        expect(code).toContain("fn algorithmPixel(pixel:vec2i,size:vec2i)->vec2i{return vec2i(pixel.x,size.y-1-pixel.y);}");
        expect(code).toContain("fn pixelOffset(direction:vec2f,distancePixels:f32)->vec2i{return vec2i(round(vec2f(direction.x,-direction.y)*distancePixels));}");
        expect(code).toContain("let noisePixel=algorithmPixel(pixel,renderSize)");
        expect(code).toContain("let random=loadNoise(pixel,size)");
        expect(code).toContain("let sampled=loadTag(pixel+pixelOffset(direction,distancePixels),size)");
        expect(code).toContain("let oppositePixel=clampPixel(pixel-pixelOffset(result.candidate.direction,1.0),size)");
    });
    ...
    it("specializes view and screen depth reconstruction", () => {
        ...
        for (const code of [viewDepth, screenDepth]) {
            ...
            expect(code).toContain("uniforms.inverseProjection*vec4f(ndcXY,depthToNdc(depth),1)");
        }
    });
});
...
describe("mesh-blending WGSL quality variants", () => {
    ...
    it.each(cases)(
        ...
        (
            ...
            if (interpolation === "sRGB") {
                expect(code).toContain("return srgbToLinear(mix(linearToSrgb(current),linearToSrgb(targetColor),amount));");
                expect(code).not.toContain("fn linearToOklab");
            } else {
                expect(code).toContain("fn linearToOklab");
                expect(code).toContain("fn oklabToLinear");
            }
        }
    );
    ...
    it("emits secondary-target evaluation only for High and Cinematic", () => {
        for (const quality of [MeshBlendQuality.Low, MeshBlendQuality.Medium]) {
            const code = shader(quality);
            expect(code).toContain("let secondary=invalidResult();");
            expect(code).not.toContain("let secondaryColor=evaluateColor");
        }
        for (const quality of [MeshBlendQuality.High, MeshBlendQuality.Cinematic]) {
            const code = shader(quality);
            expect(code).toContain("var secondary=invalidResult();");
            expect(code).toContain("let secondaryColor=evaluateColor");
...
describe("mesh-blending WGSL optional shadow path", () => {
    it("removes every base-color binding, field, load, and shadow calculation when absent", () => {
        ...
        expect(code).toContain("@group(0) @binding(3) var blueNoiseTexture:texture_2d<f32>;");
        expect(code).toContain("@group(0) @binding(4) var<uniform> uniforms:MeshBlendUniforms;");
        expect(code).toContain("evaluation.shadowAttenuation=1.0;");
    });
    ...
    it("adds the exact shifted bindings, base-color texel loads, and attenuation constants when present", () => {
        ...
        expect(code).toContain("fartherBaseColor:vec3f,conservativeNearBaseColor:vec3f,");
        expect(code).toContain("textureLoad(baseColorTexture,result.targetFarPixel,0).rgb");
        ...
        expect(code).toContain("evaluation.shadowAttenuation=shadowAttenuation(evaluation.currentColor.rgb,evaluation.samples);");
    });
});
...
describe("mesh-blending WGSL math, debug, and output contracts", () => {
    ...
    it("contains every debug-mode output contract and specializes WorldPosition", () => {
        ...
        expect(code).toContain("evaluation.worldPosition=(uniforms.inverseView*vec4f(result.currentViewPosition,1)).xyz;");
        expect(code).toContain("return vec4f(fract(primaryColor.worldPosition*0.1),1);");
        expect(shader()).not.toContain("primaryColor.worldPosition");
    });
    ...
    it("preserves exact source texels for disabled and rejected non-debug paths", () => {
        ...
        expect(code).toContain("return primaryColor.blendedColor;");
    });
    ...
    it("does not clamp HDR or negative SceneColor during either interpolation path", () => {
        for (const quality of [MeshBlendQuality.Low, MeshBlendQuality.High]) {
            const code = shader(quality);
            const interpolation = code.slice(code.indexOf("fn interpolateColor"), code.indexOf("fn luminance"));
            expect(interpolation).not.toContain("clamp");
            expect(interpolation).not.toContain("saturate");
            expect(code).toContain(
                "evaluation.blendedColor=vec4f(interpolateColor(evaluation.currentColor.rgb,evaluation.samples.targetColor.rgb,evaluation.adjustedFade),mix(evaluation.currentColor.a,evaluation.samples.targetColor.a,evaluation.adjustedFade));"
            );
        }
    });
});
