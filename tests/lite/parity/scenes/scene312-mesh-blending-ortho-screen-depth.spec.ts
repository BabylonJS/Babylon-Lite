...
const sceneConfig = getSceneConfig(312);
const referenceDir = path.resolve(__dirname, "../../../../reference/lite/scene312-mesh-blending-ortho-screen-depth");
const goldenRef = path.join(referenceDir, "babylon-ref-golden.png");

test.skip(!!sceneConfig.skipParity, "Scene 312 skipped via skipParity in scene-config.json");

test("Scene 312 — orthographic screen-depth mesh blending matches Babylon.js", async ({ page }, testInfo) => {
    await captureGolden(page.context().browser()!, { sceneId: 312 });
    await page.goto("/scene312.html");
    ...
    const region = compareRegion(actual, goldenRef, [8, 9, 13], 10);
