...
test("Scene 290 — Havok thin instances match Babylon.js", async ({ page }, testInfo) => {
    ...
    const golden = await captureGolden(browser, {
        sceneId: 290,
        queryParams: CAPTURE_QUERY,
        waitFlag: "captureReady",
        timeout: 120_000,
        settleMs: 0,
    });
    ...
    const full = compareImages(screenshotPath, golden);
    await attachCompareArtifacts(testInfo, screenshotPath, golden, REFERENCE_DIR);
