...
const sceneConfig = getSceneConfig(189);
const referenceDir = path.resolve(__dirname, "../../../../reference/lite/scene189-indirect-compute");
const golden = path.join(referenceDir, "babylon-ref-golden.png");

test("Scene 189 — GPU-produced indirect dispatch matches the reference", async ({ page }, testInfo) => {
    const browser = page.context().browser()!;
    await captureGolden(browser, { sceneId: 189, settleMs: 500 });
    await page.goto("/scene189.html");
