...
- **The per-scene bundle manifest is owned by master; never commit it from a branch.** The bundle-size baseline is **distributed**: one tracked file per scene at `lab/public/bundle/manifest/<scene>.json` (the per-scene runtime-fetched bundle sizes). The single aggregate `lab/public/bundle/manifest.json` is a generated, gitignored artifact — do **not** commit it either. A dedicated master-triggered pipeline (`azure-pipelines-bundle-manifest.yml`) measures every scene after each merge and commits the result to master; PR builds only measure and report. **Leave `lab/public/bundle/manifest/` out of your PR entirely** — if a local build dirties it, `git checkout -- lab/public/bundle/manifest` before committing. This is what keeps ~200 generated files out of every PR diff, and it is deliberate: those files were both the dominant cause of red CI (any merge made every other branch's copy stale) and the dominant cause of merge conflicts (two branches rewriting the same 200 files). It is safe because the manifest gates nothing — the ceilings live in `scene-config.json`, and the master baseline is read straight from `origin/master`. Use `pnpm validate:bundle-manifest` to see how your change moves sizes. **Bundle-size regressions are still gated by the `scene-config.json` ceilings**, which `pnpm build:bundle-scenes` enforces byte-exactly and which no automation may raise.
