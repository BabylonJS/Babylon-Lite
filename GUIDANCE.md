...
- **The per-scene bundle manifest is CI-owned; you do not have to regenerate it.** The bundle-size baseline is **distributed**: one tracked file per scene at `lab/public/bundle/manifest/<scene>.json` (the per-scene runtime-fetched bundle sizes). The single aggregate `lab/public/bundle/manifest.json` is a generated, gitignored artifact — do **not** commit it. The PR-validation Bundle Size job measures every scene and pushes the regenerated per-scene files back to the PR branch as a bot commit, so the deltas still land in the diff for review. A stale manifest therefore no longer fails CI — which is the point: a shared-module change moves bytes in most scenes, so an exact-equality gate turned every open PR red as soon as any other shared-code PR merged. If you do regenerate manifests locally, commit only the files for the filtered scenes relevant to the change and leave unrelated manifests untouched; `pnpm validate:bundle-manifest:strict` reports whether they are fully in sync. **Bundle-size regressions are still gated by the `scene-config.json` ceilings**, which `pnpm build:bundle-scenes` enforces byte-exactly and which no automation may raise.
